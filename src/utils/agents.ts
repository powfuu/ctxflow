...
export const ALL_CONTEXT_FILES: string[] = [...new Set(AGENTS.map(a => a.file))];

export async function isProjectInitialised(cwd: string): Promise<boolean> {
  const checks = await Promise.all(
    ALL_CONTEXT_FILES.map(f => pathExists(join(cwd, f))),
  );

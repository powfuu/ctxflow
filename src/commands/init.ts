...
import { AGENTS, AgentConfig } from '../utils/agents.js';

const TODAY = new Date().toISOString().split('T')[0];

async function pickAgents(): Promise<number[] | null> {
  const items = AGENTS.map(a => a.label);
  let cursor = 0;
  const selected = new Set<number>([0]); // Claude Code pre-selected
  ...
  function render(): void {
    ...
    process.stdout.write(
      `  ${chalk.cyan('Which AI agents do you use?')}  ` +
  ...
  return new Promise<number[] | null>((resolve) => {
    ...
    const onData = (chunk: Buffer): void => {
      ...
      if (key === '\x1b[A' || key === 'k') { cursor = (cursor - 1 + items.length) % items.length; render(); return; }
      if (key === '\x1b[B' || key === 'j') { cursor = (cursor + 1) % items.length; render(); return; }
...
async function confirmOverwrite(_files: string[]): Promise<boolean> {
  return new Promise<boolean>((resolve) => {
    ...
    stdin.once('data', (chunk) => {
      ...
      if (key === '\x1b') { resolve(false); return; }
      if (key === '\r' || key === '\n') { resolve(true); return; }
      if (key.toLowerCase() === 'y') { resolve(true); return; }
      resolve(false);
    });
  });
}

export async function initCommand(cwd = process.cwd()): Promise<boolean> {
  ...
  for (const i of indices) {
    const agent = AGENTS[i] as AgentConfig;
    if (!seen.has(agent.file)) seen.set(agent.file, agent);
  }
  const toCreate = [...seen.values()];

  const existing: string[] = [];
  for (const agent of toCreate) {
    if (await pathExists(join(cwd, agent.file))) existing.push(agent.file);
  }

...
async function confirmOverwrite(): Promise<boolean> {
  return new Promise<boolean>((resolve) => {
    ...
    process.stdout.write(
      `  ${chalk.yellow('CLAUDE.md already exists.')} ${chalk.white('Overwrite?')}` +
    ...
    stdin.once('data', (chunk) => {
      ...
      process.stdout.write('\n');
      if (key === '\x03') process.exit(0);          // Ctrl+C → exit
      if (key === '\x1b') { resolve(false); return; } // Esc → no
      if (key === '\r' || key === '\n') { resolve(true); return; } // Enter → yes
